-- ============================================================
-- CodeCanvas — Full Database Schema Migration
-- Run this in your Supabase SQL Editor
-- ============================================================

-- ─── PROFILES (extends auth.users with role + school) ────────
...
CREATE POLICY "Teachers can view student profiles" ON public.profiles
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE p.id = auth.uid() AND p.role IN ('teacher', 'hod', 'admin')
    )
  );
...
CREATE POLICY "Teachers manage their classrooms" ON public.classrooms
...
CREATE POLICY "Students can see own enrollments" ON public.enrollments
  FOR SELECT USING (student_id = auth.uid());

CREATE POLICY "Teachers can see their class enrollments" ON public.enrollments
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Teachers can remove students" ON public.enrollments
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.classrooms c
      WHERE c.id = classroom_id AND c.teacher_id = auth.uid()
...
CREATE POLICY "Students view assignments for their classes" ON public.assignments
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.enrollments e
...
CREATE POLICY "Students submit and view own submissions" ON public.submissions
...
CREATE POLICY "Users can insert own traces" ON public.trace_history
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users can view own traces" ON public.trace_history
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Teachers can view student traces" ON public.trace_history
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.profiles p
      WHERE p.id = auth.uid() AND p.role IN ('teacher', 'hod', 'admin')
...
CREATE POLICY "Anyone can view shared traces" ON public.shared_traces
  FOR SELECT USING (true);

CREATE POLICY "Authenticated users can share" ON public.shared_traces
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users increment view count" ON public.shared_traces
...
-- Update streak for a user (call after each trace)
...
BEGIN
  SELECT last_active INTO last_active_date
  ...
  IF last_active_date IS NULL OR last_active_date < today - INTERVAL '1 day' THEN
    ...
    END IF;
  ELSE
    -- Already active today, just update last_active
    UPDATE public.profiles SET last_active = today WHERE id = user_id_param;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Upsert student_progress with increment (avoids race conditions)
CREATE OR REPLACE FUNCTION public.upsert_progress(
  user_id_param UUID,
  algorithm_type_param TEXT
) RETURNS VOID AS $$
BEGIN
  ...
  ON CONFLICT (user_id, algorithm_type)
  DO UPDATE SET
